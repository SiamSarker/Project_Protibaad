...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $sql_del = "DELETE FROM `crimes` WHERE `id`='$id'";
    $result_del = mysqli_query($conn, $sql_del);
    if ($result_del == TRUE) {
        echo "<script>alert('Record deleted successfully.')</script>";
        header('Location=crime_list.php');
    }else{
...
if (isset($_GET['approve'])) {
    $id = $_GET['approve'];
    $sql_approve = "UPDATE `crimes` SET `approve_status`='1' WHERE `id`='$id'";
        ...
        <table class="table">
            ...
            <?php while($row =  mysqli_fetch_array($result)) { ?>
                <tr>
                    ...
                    <td>
                        <a class="btn btn-info" href="crime_list.php?delete=<?php echo $row['id']; ?>">Delete</a>
                        <?php if($row['approve_status'] == '1') { ?>
                            <h3>Approved</h3>
                        <?php } else { ?>
                            <a class="btn btn-info" href="crime_list.php?approve=<?php echo $row['id']; ?>">Approve</a>
                        <?php } ?>
                        <a class="btn btn-info" href="crime_details.php?id=<?php echo $row['id']; ?>">Details</a>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
