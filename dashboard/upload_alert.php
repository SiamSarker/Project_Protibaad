...
if (isset($_SESSION['id'])) {
    ...
    $validExt = array ('jpeg','jpg','png', 'mp4','mov','avi','mkv','pdf', 'txt', 'doc', 'docx');
    if (empty($file)) {
...
echo "<script>alert('file size is not proper');</script>";
        ...
        if(move_uploaded_file($_FILES['file']['tmp_name'], $folder.$notefile)) {
            ...
            if (mysqli_affected_rows($conn) > 0) {
                echo "<script> alert('Alert uploaded successfully.It will be published after admin approves it');
                window.location.href='alerts.php';</script>";
            }
            else {
...
?>
	...
	<div class="form-group">
		<label for="post_title">Crime Title</label>
		<input type="text" name="title" class="form-control" placeholder="Eg: A fighting is happening in front of uiu"  value = "<?php if(isset($_POST['upload'])) {
            echo $file_title; } ?>" required="">
	</div>

	<div class="form-group">
		<label for="post_tags">Description</label>
		<input type="text" name="description" class="form-control" placeholder="Eg: Two groups are fighting with each other ...." value="<?php if(isset($_POST['upload'])) {
            echo $file_description;  } ?>" required="" ">
	</div>

	 <div class="form-group">
        <label for="post_image">Select File</label><font color="brown"> (allowed file type: 'jpeg','jpg','png','png', 'mp4','mov','avi','mkv''pdf', 'txt', 'doc', 'docx' | allowed maximum size: 300 mb ) </font>
		<input type="file" name="file"> 
     </div>

<button type="submit" name="upload" class="btn btn-primary" value="Upload Note">Upload Alert</button>
<br>
<br>
</form>
</div>
